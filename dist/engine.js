...
class Game {
 ...
 emit(type,text,data={}){const e={id:this.id(),type,text,round:this.s.round,lane:this.s.lane,...data};this.s.events.push(e);this.s.log.push(e);this.s.log=this.s.log.slice(-240);}
 ...
 enabled(u){return u.alive&&!this.flag(u,'silence')&&!this.flag(u,'stun');}
 ...
 stats(u){
  ...
  r.attack=Math.max(0,r.attack);if(u.k==='sven'&&this.enabled(u))r.cleave+=Math.floor(r.attack/2);r.health=Math.max(1,r.health);r.hp=Math.max(0,r.health-u.damage);return r;
 }
 itemStats(k){
 ...
 placementPosition(p,l,pos=null,exclude=null){
  ...
  if(pos===-1){for(const u of this.all(null,l))if(u!==exclude)u.pos++;return 0;}
  return pos;
 }
 ...
 compactLane(l){
  ...
  const units=this.all(null,l),positions=[...new Set(units.map(u=>u.pos))].sort((a,b)=>a-b);
 ...
 damage(u,n,pierce=false,source=null){if(!u?.alive)return 0;const st=this.stats(u);if(st.immune)return 0;const d=Math.max(0,n-(pierce?0:st.armor));if(n<=0)return 0;u.damage+=d;if(d)this.emit('damage',this.card(u.k).name+'受到 '+d+' 点伤害',{unit:u.uid,amount:d,source});return d;}
 ...
 condemn(u){if(u)this.buff(u,{condemned:1},'death');}
 returnHero(u){if(!u?.hero)return;u.alive=false;u.lane=-1;u.readyRound=this.s.round+1;u.mods=u.mods.filter(m=>m.until==='permanent');u.target=null;this.emit('return',this.card(u.k).name+'返回泉水',{unit:u.uid});}
 move(u,l,pos=null){if(!u?.alive)return;if(l<0||l>2)throw Error('无效战线');const old=u.lane;pos=this.placementPosition(u.owner,l,pos,u);u.lane=l;u.pos=pos;this.resetArrow(u);const opposite=this.at(1-u.owner,l,pos);if(opposite&&!opposite.target)opposite.arrow=0;if(this.hasImp(0,l,'glyph_of_confusion')||this.hasImp(1,l,'glyph_of_confusion'))this.buff(u,{stun:1},'round');this.emit('move',this.card(u.k).name+'转移至'+['上路','中路','下路'][l],{unit:u.uid,from:old,to:l});}
 swap(a,b){if(!a||!b||a.owner!==b.owner||a.lane!==b.lane)throw Error('只能交换同路友方单位');[a.pos,b.pos]=[b.pos,a.pos];a.target=null;b.target=null;}
 battle(a,b){if(!a?.alive||!b?.alive||a===b)return;const sa=this.stats(a),sb=this.stats(b);let da=0,db=0;if(!this.flag(a,'stun')&&!this.flag(a,'disarm'))da=this.damage(b,sa.attack+(b.hero?sa.heroBonus:0),sa.pierce,a.uid);if(!this.flag(b,'stun')&&!this.flag(b,'disarm'))db=this.damage(a,sb.attack+(a.hero?sb.heroBonus:0),sb.pierce,b.uid);if(da)this.onBattleHit(a,b);if(db)this.onBattleHit(b,a);if(da&&sb.retaliate)this.damage(a,sb.retaliate);if(db&&sa.retaliate)this.damage(b,sa.retaliate);this.sweep();}
 ...
 combatForecast(l=this.s.lane){const result={tower:[0,0],units:{}};const active=this.all(null,l);for(const u of active)result.units[u.uid]={damage:0,heal:Math.min(u.damage,this.stats(u).regen)};
  const hit=(u,n,pierce=false)=>{if(!u)return;const st=this.stats(u);if(st.immune||n<=0)return;result.units[u.uid].damage+=Math.max(0,n-(pierce?0:st.armor));};
  for(const u of active){if(this.flag(u,'stun')||this.flag(u,'disarm'))continue;const st=this.stats(u),t=this.target(u);if(t){hit(t,st.attack+(t.hero?st.heroBonus:0),st.pierce);if(st.attack>0){for(const v of this.neighbors(t))hit(v,st.cleave);hit(u,this.stats(t).retaliate);result.tower[1-u.owner]+=Math.max(0,st.siege-this.towerArmor(1-u.owner,l));}}else{const n=st.attack+st.towerBonus;result.tower[1-u.owner]+=Math.max(0,n-this.towerArmor(1-u.owner,l));if(n>0)hit(u,2*this.hasImp(1-u.owner,l,'burning_oil'));}}
  return result;
 }
 combat(){
  if(this.s.phase!=='action')throw Error('只能在行动阶段结算战斗');
  const l=this.s.lane;this.s.phase='combat';const f=this.combatForecast(l),active=[...this.all(null,l)];this.emit('combat',['上路','中路','下路'][l]+'战斗结算',{attacks:active.filter(u=>!this.flag(u,'stun')&&!this.flag(u,'disarm')&&this.stats(u).attack>0).map(u=>({unit:u.uid,target:this.target(u)?.uid||null,owner:u.owner}))});
  for(const u of active){this.heal(u,f.units[u.uid].heal);const d=f.units[u.uid].damage;u.damage+=d;if(d)this.emit('damage',this.card(u.k).name+'受到 '+d+' 点战斗伤害',{unit:u.uid,amount:d});}
  ...
  for(const u of active){const st=this.stats(u);if(this.flag(u,'stun')||this.flag(u,'disarm')||!st.attack)continue;const t=this.target(u);if(t&&!this.stats(t).immune&&st.attack+(t.hero?st.heroBonus:0)>(st.pierce?0:this.stats(t).armor))this.onBattleHit(u,t);if(!t)u.hitType='Tower';}
 ...
 play(p,handId,t=[]){const backup=clone(this.s);try{this.s.events=[];const pl=this.s.players[p],idx=pl.hand.findIndex(h=>h.uid===Number(handId)),h=pl.hand[idx];const why=this.canPlay(p,h);if(why)throw Error(why);const c=this.card(h.k);this.validateTargets(c,p,t);pl.hand.splice(idx,1);if(c.type!=='Item')this.s.lanes[this.s.lane].mana[p]-=c.mana;
   ...
   if(h.sabotage)this.towerDamage(p,Math.floor(this.random()*3),6);this.afterCard(p,c);this.sweep();if(this.s.pendingCombat){delete this.s.pendingCombat;if(this.s.winner===null)this.combat();return this.s;}if(this.s.winner===null&&this.s.phase==='action'){this.s.passes=0;this.s.turn=c.initiative?p:1-p;if(c.initiative)this.s.initiative=p;}return this.s;
  }catch(e){this.s=backup;throw e;}}
 purge(u){u.mods=u.mods.filter(m=>!m.stun&&!m.silence&&!m.disarm&&!m.poison&&!m.unprepared&&!(m.attack<0)&&!(m.armor<0));}
 afterCard(p,c){const l=this.s.lane;for(const u of this.all(p)){if(!this.enabled(u))continue;const local=u.lane===l;
 ...
 if(c.type==='Spell')for(let n=0;n<this.hasImp(1-p,l,'nether_ward');n++)this.towerDamage(p,l,3);
 }
}
