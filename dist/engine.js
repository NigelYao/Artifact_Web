...
class Game {
 ...
 combat(){
  ...
  for(const attack of f.attacks){const u=this.get(attack.unit),st=this.stats(u),t=this.get(attack.target);if(t&&f.hitDamage[u.uid]>0)this.onBattleHit(u,t);if(!t)u.hitType='Tower';this.consumeShadow(u);}
  for(const id of f.roots)this.buff(this.get(id),{root:1},'round');
  this.sweep();if(this.s.winner!==null)return;
  for(const u of this.all(null,l)){if(this.enabled(u)){if(u.k==='rampaging_hellbear')this.buff(u,{attack:4});if(u.k==='satyr_duelist')this.buff(u,{attack:2});if(u.k==='savage_wolf')this.buff(u,{attack:1,health:2});if(u.k==='selfish_cleric')this.heal(u,999);if(u.k==='cursed_satyr')this.spawn('zombie',1-u.owner,l);if(u.k==='red_mist_pillager'&&u.hitType==='Tower')this.spawn(u.k,u.owner,l);if(u.k==='rebel_instigator'&&u.hitType==='Creep')this.spawn(u.k,u.owner,l);}
