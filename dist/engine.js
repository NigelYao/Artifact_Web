...
const ZLANE=l=>['上路','中路','下路'][l],ELANE=l=>['Top Lane','Middle Lane','Bottom Lane'][l];
const ZSIDE=p=>p?'夜魇':'天辉',ESIDE=p=>p?'Dire':'Radiant';
const LANE=l=>T(ZLANE(l),ELANE(l)),SIDE=p=>T(ZSIDE(p),ESIDE(p));
const NM=c=>T(c.name,c.en||''),EN=c=>c.en||c.name;
const SKILL=(k,name)=>{const a=(root.ArtifactHeroSkills?.all?.[k]||[]).find(x=>x.name===name);return T(name,a?.en||'')};
const SKEN=(k,name)=>(root.ArtifactHeroSkills?.all?.[k]||[]).find(x=>x.name===name)?.en||name;
...
class Game {
 ...
 passive(u,name,data={}){this.emit('passive',NM(this.card(u.k))+' · '+SKILL(u.k,name),{unit:u.uid,card:u.k,owner:u.owner,skill:name,...data});}
 ...
 placementPosition(p,l,pos=null,exclude=null){
  ...
  if(!Number.isInteger(pos)||pos< -1)throw Error(T('请选择有效位置','Choose a valid position'));
  if(this.all(p,l).some(u=>u!==exclude&&u.pos===pos))throw Error(T('这个位置已被占用','That position is already occupied'));
 ...
 spawn(k,p,l,pos=null,{hero=false,copy=false}={}){
  ...
  this.emit('summon',T(this.card(k).name+'进入'+ZLANE(l),EN(this.card(k))+' enters '+ELANE(l)),{unit:u.uid});return u;
 }
 newGame(deck,enemy,seed=Date.now()){
  ...
  for(let l=0;l<3;l++)this.assignArrows(l);this.beforeAction(0);this.emit('round',T('第 1 回合 · 上路开始','Round 1 · '+ELANE(0)+' begins'));return this.s;
 }
 validateDeck(d){const e=[];if(!d||!Array.isArray(d.heroes)||!Array.isArray(d.main)||!Array.isArray(d.items))return [T('牌组格式错误','Invalid deck format')];if(d.heroes.length!==5||new Set(d.heroes).size!==5)e.push(T('需要 5 位不同英雄','Requires 5 different heroes'));if(d.main.length+15<40)e.push(T('主牌组含专属牌至少 40 张','Main deck needs at least 40 cards including signatures'));if(d.items.length<9)e.push(T('物品牌组至少 9 张','Item deck needs at least 9 cards'));const colors=new Set(d.heroes.map(k=>this.card(k)?.color));for(const k of d.heroes)if(this.card(k)?.type!=='Hero')e.push(T('英雄选择无效','Invalid hero selection'));for(const list of [d.main,d.items]){const count={};for(const k of list){const c=this.card(k);if(!c){e.push(T('未知卡牌','Unknown card'));continue;}count[k]=(count[k]||0)+1;if(count[k]>3)e.push(T(c.name+'超过 3 张',EN(c)+' exceeds 3 copies'));if(list===d.main&&c.requiredHero&&!d.heroes.includes(c.requiredHero))e.push(T(c.name+'需要在牌组中加入'+this.card(c.requiredHero).name,EN(c)+' requires '+EN(this.card(c.requiredHero))+' in the deck'));if(list===d.main&&(c.signatureOf||c.token||['Item','Hero'].includes(c.type)||!colors.has(c.color)))e.push(T(c.name+'无法加入主牌组',EN(c)+' cannot go in the main deck'));if(list===d.items&&(c.type!=='Item'||c.itemType==='Consumable'))e.push(T(c.name+'无法加入物品牌组',EN(c)+' cannot go in the item deck'));}}return [...new Set(e)];}
 draw(p,n){const pl=this.s.players[p];for(let i=0;i<n;i++){if(!pl.deck.length)break;let lock=this.all(1-p).filter(u=>this.equipment(u,'claszureme_hourglass')).length;const c=pl.deck.pop();pl.hand.push({uid:this.id(),k:typeof c==='string'?c:c.k,charges:typeof c==='string'?0:c.charges||0,lock});}this.emit('draw',T(`${ZSIDE(p)}抽取 ${n} 张牌`,`${ESIDE(p)} draws ${n} cards`));}
 heal(u,n){if(!u?.alive)return;const before=u.damage;u.damage=Math.max(0,u.damage-n);if(before!==u.damage)this.emit('heal',T(this.card(u.k).name+'回复 '+(before-u.damage),EN(this.card(u.k))+' heals '+(before-u.damage)),{unit:u.uid,amount:before-u.damage});}
 healTower(p,l,n){const t=this.s.lanes[l].towers[p];t.hp=Math.min(t.max,t.hp+n);}
 damage(u,n,pierce=false,source=null){if(n<=0)return 0;if(!this.selectable(u))return 0;const st=this.stats(u);if(st.immune)return 0;const d=this.absorb(u,Math.max(0,n-(pierce?0:st.armor)));if(n<=0)return 0;u.damage+=d;if(d)this.emit('damage',T(this.card(u.k).name+'受到 '+d+' 点伤害',EN(this.card(u.k))+' takes '+d+' damage'),{unit:u.uid,amount:d,source});return d;}
 towerDamage(p,l,n,pierce=false){const t=this.s.lanes[l].towers[p];const d=Math.max(0,n-(pierce?0:this.towerArmor(p,l)));t.hp-=d;t.damageRound+=d;this.emit('tower',T(`${ZSIDE(p)}${ZLANE(l)}受到 ${d} 点伤害`,`${ESIDE(p)} takes ${d} damage in ${ELANE(l)}`),{owner:p,lane:l,amount:d});}
 checkWin(){if(this.s.winner!==null)return;const lost=[false,false];for(let p=0;p<2;p++){for(let l=0;l<3;l++){const t=this.s.lanes[l].towers[p];if(t.hp<=0){if(t.fallen){lost[p]=true;}else{t.fallen=true;t.hp=80;t.max=80;this.emit('destroy',T(`${ZSIDE(p)}${ZLANE(l)}防御塔倒塌，遗迹出现`,`${ESIDE(p)}'s ${ELANE(l)} tower falls — the Ancient is exposed`),{owner:p,lane:l});}}}if(this.s.lanes.filter(l=>l.towers[p].fallen).length>=2)lost[p]=true;}if(lost[0]||lost[1]){this.s.winner=lost[0]&&lost[1]?2:lost[0]?1:0;this.s.phase='ended';this.emit('victory',T(this.s.winner===2?'双方同时摧毁目标 · 平局':this.s.winner===0?'天辉胜利':'夜魇胜利',this.s.winner===2?'Both objectives destroyed at once · Draw':this.s.winner===0?'Radiant victory':'Dire victory'));}}
 sweep(){
  ...
  for(let iteration=0;iteration<20;iteration++){for(let l=0;l<3;l++)this.compactLane(l);const dead=this.all().filter(u=>this.stats(u).hp<=0||this.flag(u,'condemned'));if(!dead.length)break;
   for(const u of dead){if(!u.alive)continue;const shield=u.mods.find(m=>m.deathShield>0);if(shield&&!this.flag(u,'ignoreShield')){shield.deathShield--;u.damage=Math.max(0,this.stats(u).health-1);u.mods=u.mods.filter(m=>!m.condemned);continue;}
    ...
    const p=u.owner,l=u.lane,mine=this.mineOn(u),neighbors=this.neighbors(u),opponents=this.all(1-p,l),watchers=this.all(null,l),blockers=new Set(opponents.filter(v=>this.target(v)?.uid===u.uid).map(v=>v.uid));u.alive=false;
    let bounty=(u.hero?5:1)+this.value(u,'bounty')-(this.equipment(u,'revtel_signet_ring')?3:0);this.s.players[1-p].gold+=Math.max(0,bounty);this.emit('death',T(this.card(u.k).name+'阵亡',EN(this.card(u.k))+' dies'),{unit:u.uid});
    if(mine)this.detonate(mine,'death');
  ...
  }this.s.mines=(this.s.mines||[]).filter(m=>{const u=this.get(m.unit);return u&&u.alive;});for(const u of this.all())if(priorTargets.get(u.uid)!==this.target(u)?.uid&&(!u.target||!this.get(u.target)?.alive))this.resetArrow(u);this.checkWin();
 }
 condemn(u){if(this.selectable(u))this.buff(u,{condemned:1},'death');}
 mineOn(u){return (this.s.mines||[]).find(m=>m.unit===u?.uid);}
 mineTrigger(u,cause,byOwner){const m=u&&this.mineOn(u);if(m&&m.owner!==byOwner)this.detonate(m,cause);return m;}
 detonate(m,cause='trigger'){const u=this.get(m.unit),l=u&&u.lane>=0?u.lane:m.lane,pos=u?u.pos:m.pos;this.s.mines=this.s.mines.filter(x=>x!==m);this.emit('mine',T('感应地雷引爆','Proximity Mine detonates'),{owner:m.owner,lane:l,pos,unit:m.unit,cause});for(const v of this.all(1-m.owner,l))if(Math.abs(v.pos-pos)<=1)this.damage(v,8,false,m.uid);}
 returnHero(u){if(!u?.hero)return;if(!this.canMove(u))throw Error(T('缠绕或翻牌期间无法回城','Cannot return to the Fountain while rooted or flipped'));u.alive=false;u.lane=-1;u.readyRound=this.s.round+1;u.mods=u.mods.filter(m=>m.until==='permanent');u.target=null;this.emit('return',T(this.card(u.k).name+'返回泉水',EN(this.card(u.k))+' returns to the Fountain'),{unit:u.uid});}
 move(u,l,pos=null){if(!u?.alive)return;if(!this.canMove(u))throw Error(T('缠绕或翻牌期间无法移动','Cannot move while rooted or flipped'));if(l<0||l>2)throw Error(T('无效战线','Invalid lane'));const old=u.lane;pos=this.placementPosition(u.owner,l,pos,u);u.lane=l;u.pos=pos;this.resetArrow(u);const opposite=this.at(1-u.owner,l,pos);if(opposite&&!opposite.target)opposite.arrow=0;if(this.hasImp(0,l,'glyph_of_confusion')||this.hasImp(1,l,'glyph_of_confusion'))this.buff(u,{stun:1},'round');this.emit('move',T(this.card(u.k).name+'转移至'+ZLANE(l),EN(this.card(u.k))+' moves to '+ELANE(l)),{unit:u.uid,from:old,to:l});}
 swap(a,b){if(!a||!b||a.owner!==b.owner||a.lane!==b.lane)throw Error(T('只能交换同路友方单位','Can only swap allied units in the same lane'));if(!this.canMove(a)||!this.canMove(b))throw Error(T('缠绕或翻牌期间无法移动','Cannot move while rooted or flipped'));[a.pos,b.pos]=[b.pos,a.pos];a.target=null;b.target=null;}
 battle(a,b){if(!a?.alive||!b?.alive||a===b)return;const sa=this.stats(a),sb=this.stats(b),aa=this.canAttack(a)&&this.attackable(b)&&sa.attack>0,ab=this.canAttack(b)&&this.attackable(a)&&sb.attack>0;let da=0,db=0;if(aa)da=this.damage(b,sa.attack+(b.hero?sa.heroBonus:0),sa.pierce,a.uid);if(ab)db=this.damage(a,sb.attack+(a.hero?sa.heroBonus:0),sb.pierce,b.uid);if(da)this.onBattleHit(a,b);if(db)this.onBattleHit(b,a);if(da&&sb.retaliate)this.damage(a,sb.retaliate);if(db&&sa.retaliate)this.damage(b,sa.retaliate);for(const [attacker,target,attacked] of [[a,b,aa],[b,a,ab]])if(attacked){if(target.k==='dark_willow_bramble'){this.damage(attacker,1,true,target.uid);this.buff(attacker,{root:1},'round');}this.consumeShadow(attacker);}this.countAttack(a,aa);this.countAttack(b,ab);for(const x of [a,b])this.expireSoldier(x);this.sweep();}
 countAttack(u,attacked){if(!attacked||!u?.alive)return;if(u.k==='monkey_king'&&this.enabled(u)){u.charges=(u.charges||0)+1;this.emit('passive',NM(this.card(u.k))+' · '+SKILL(u.k,'棒击蓄势')+T(' · 能量 '+u.charges,' · Energy '+u.charges),{unit:u.uid,card:u.k,owner:u.owner,skill:'棒击蓄势'});}if(u.k==='monkey_soldier')u.attacks=(u.attacks||0)+1;}
 expireSoldier(u){if(u?.alive&&u.k==='monkey_soldier'&&(u.attacks||0)>=2){u.alive=false;u.lane=-1;u.target=null;this.emit('expire',T(this.card(u.k).name+'出击完成，消散',EN(this.card(u.k))+' finishes its attacks and vanishes'),{unit:u.uid});}}
 ...
 combatForecast(l=this.s.lane){const result={tower:[0,0],units:{},attacks:[],roots:[],hitDamage:{}};const active=this.all(null,l);for(const u of active)result.units[u.uid]={damage:0,shieldSpent:0,heal:this.flag(u,'flipped')?0:Math.min(u.damage,this.stats(u).regen)};
  ...
  for(const u of active){if(!this.canAttack(u))continue;const st=this.stats(u),t=this.target(u);if(t&&!this.flag(t,'flipped')){if(!this.attackable(t))continue;result.hitDamage[u.uid]=hit(t,st.attack+(t.hero?st.heroBonus:0),st.pierce,true)||0;if(st.attack>0){result.attacks.push({unit:u.uid,target:t.uid,owner:u.owner});for(const v of this.neighbors(t))hit(v,st.cleave,false,true);hit(u,this.stats(t).retaliate);if(t.k==='dark_willow_bramble'){hit(u,1,true);result.roots.push(u.uid);}result.tower[1-u.owner]+=Math.max(0,st.siege-this.towerArmor(1-u.owner,l));}}else{if(u.k==='monkey_soldier')continue;if(t&&!u.hero)continue;const n=st.attack+st.towerBonus,amount=Math.max(0,n-this.towerArmor(1-u.owner,l))*(t ? .5 : 1);result.tower[1-u.owner]+=amount;if(n>0){result.attacks.push({unit:u.uid,target:null,owner:u.owner});hit(u,2*this.hasImp(1-u.owner,l,'burning_oil'));}}}
  return result;
 }
 combat(){
  if(this.s.phase!=='action')throw Error(T('只能在行动阶段结算战斗','Combat can only resolve during the action phase'));
  const l=this.s.lane;this.s.phase='combat';const f=this.combatForecast(l),active=[...this.all(null,l)];this.emit('combat',T(ZLANE(l)+'战斗结算',ELANE(l)+' combat resolves'),{attacks:f.attacks});
  for(const u of active){this.heal(u,f.units[u.uid].heal);const d=f.units[u.uid].damage;this.absorb(u,f.units[u.uid].shieldSpent);u.damage+=d;if(d)this.emit('damage',T(this.card(u.k).name+'受到 '+d+' 点战斗伤害',EN(this.card(u.k))+' takes '+d+' combat damage'),{unit:u.uid,amount:d});}
  for(let p=0;p<2;p++){const t=this.s.lanes[l].towers[p];t.hp-=f.tower[p];t.damageRound+=f.tower[p];if(f.tower[p])this.emit('tower',T(`${ZSIDE(p)}防御建筑受到 ${f.tower[p]} 点战斗伤害`,`${ESIDE(p)}'s tower takes ${f.tower[p]} combat damage`),{owner:p,lane:l,amount:f.tower[p]});}
  ...
  this.sweep();if(this.s.winner!==null)return;
  if(l===2){this.s.phase='shop';this.openShop();}else{this.s.lane++;this.s.turn=this.s.initiative;this.s.passes=0;this.s.phase='action';this.beforeAction(this.s.lane);this.emit('lane',T(ZLANE(this.s.lane)+'行动阶段',ELANE(this.s.lane)+' action phase'));}
 }
 openShop(){const items=Object.values(this.cards).filter(c=>c.type==='Item'&&c.itemType!=='Consumable'&&!c.token);const consumables=['healing_salve','town_portal_scroll','potion_of_knowledge'];for(let p=0;p<2;p++){const pl=this.s.players[p];const old=pl.shop;pl.shop={secret:this.pick(items).key,deck:old?.hold?old.deck:(pl.itemDeck[0]||null),consumable:this.pick(consumables),hold:false,consumed:false,secretBought:false};}this.emit('shop',T('商店开张 · 使用战斗获得的金币购买物品','Shop opens · spend gold earned in combat'));}
 buy(p,slot){if(this.s.phase!=='shop')throw Error(T('现在不是购物阶段','Not the shopping phase'));if(!['deck','secret','consumable'].includes(slot))throw Error(T('无效商品','Invalid shop slot'));const pl=this.s.players[p],shop=pl.shop,k=shop[slot],c=this.card(k);if(!c||slot==='secret'&&shop.secretBought||slot==='consumable'&&shop.consumed)throw Error(T('本轮该商品已售罄','Sold out this round'));const price=slot==='secret'&&pl.freeShop?0:c.gold;if(pl.gold<price)throw Error(T('金币不足','Not enough gold'));this.s.events=[];pl.gold-=price;pl.hand.push({uid:this.id(),k,lock:0});if(slot==='deck'){pl.itemDeck.splice(pl.itemDeck.indexOf(k),1);shop.deck=pl.itemDeck[0]||null;}else if(slot==='secret')shop.secretBought=true;else shop.consumed=true;this.emit('buy',T(`${ZSIDE(p)}购买了 ${c.name}`,`${ESIDE(p)} buys ${EN(c)}`),{card:k});}
 hold(p){const pl=this.s.players[p];if(this.s.phase!=='shop'||pl.shop.hold||!pl.shop.deck||pl.gold<1)throw Error(T('无法保留商品','Cannot hold the item'));this.s.events=[];pl.gold--;pl.shop.hold=true;this.emit('buy',T('支付 1 金币，保留物品牌组商品','Pays 1 gold to hold the item-deck offer'));}
 startRound(){if(this.s.phase!=='shop')throw Error(T('先完成本轮购物','Finish shopping first'));this.s.round++;this.s.lane=0;this.s.passes=0;this.s.events=[];
  ...
  for(const u of this.all().filter(u=>u.expiresRound&&u.expiresRound<this.s.round)){u.alive=false;u.lane=-1;this.emit('expire',T(this.card(u.k).name+'消散',EN(this.card(u.k))+' dissipates'),{unit:u.uid});}
  ...
  this.s.phase='deploy';this.emit('round',T('第 '+this.s.round+' 回合 · 部署英雄','Round '+this.s.round+' · Deploy heroes'));
 }
 ready(p){return this.s.units.filter(u=>u.owner===p&&u.hero&&!u.copy&&!u.alive&&u.readyRound<=this.s.round);}
 deploy(id,l,pos=null){if(this.s.phase!=='deploy')throw Error(T('现在不是部署阶段','Not the deployment phase'));const u=this.get(id);if(!u||!u.hero||u.copy||u.alive||u.readyRound>this.s.round)throw Error(T('英雄尚未复活','Hero has not respawned yet'));if(!Number.isInteger(l)||l<0||l>2)throw Error(T('无效战线','Invalid lane'));pos=this.placementPosition(u.owner,l,pos,u);u.damage=0;u.alive=true;u.lane=l;u.pos=pos;this.resetArrow(u);const opposite=this.at(1-u.owner,l,pos);if(opposite&&!opposite.target)opposite.arrow=0;if(this.hasImp(0,l,'glyph_of_confusion')||this.hasImp(1,l,'glyph_of_confusion'))this.buff(u,{stun:1},'round');this.emit('summon',T(this.card(u.k).name+'部署至'+ZLANE(l),EN(this.card(u.k))+' deploys to '+ELANE(l)),{unit:u.uid});}
 finishDeployment(){if(this.ready(0).length||this.ready(1).length)throw Error(T('请部署所有待命英雄','Deploy all waiting heroes'));for(let l=0;l<3;l++){this.assignArrows(l);for(let p=0;p<2;p++)this.s.lanes[l].mana[p]=this.maxMana(p,l);}this.s.phase='action';this.s.turn=this.s.initiative;this.beforeAction(0);this.emit('lane',T('上路行动阶段',ELANE(0)+' action phase'));}
 pass(p){if(this.s.phase!=='action'||this.s.turn!==p)throw Error(T('尚未轮到你行动','Not your turn to act'));this.s.events=[];this.emit('pass',T(`${ZSIDE(p)}让过`,`${ESIDE(p)} passes`));if(this.s.passes===0)this.s.initiative=p;this.s.passes++;if(this.s.passes>=2)this.combat();else this.s.turn=1-p;}
 canPlay(p,h){const c=this.card(h?.k);if(!c)return T('找不到这张牌','Card not found');if(this.s.phase!=='action'||this.s.turn!==p)return T('等待你的行动机会','Wait for your action');if(h.lock>0)return T('此牌仍被锁定 '+h.lock+' 回合','This card is locked for '+h.lock+' more rounds');if(c.type==='Item')return '';const l=this.s.lane;if(c.requiredHero&&!this.named(p,c.requiredHero,l).some(u=>c.key!=='pangolier_gyroshell'||this.canMove(u)))return T('本路需要可行动的'+this.card(c.requiredHero).name+(c.key==='pangolier_gyroshell'?'且不能被缠绕':''),'Requires a ready '+EN(this.card(c.requiredHero))+' in this lane'+(c.key==='pangolier_gyroshell'?' (and not rooted)':''));if(this.s.lanes[l].mana[p]<c.mana)return T('当前战线魔力不足','Not enough mana in this lane');if(this.hasImp(p,l,'the_oath')&&['Spell','Creep'].includes(c.type))return T('誓约阻止施放法术和小兵','The Oath prevents playing spells and creeps');if(!this.all(p,l).some(u=>u.hero&&this.card(u.k).color===c.color&&this.enabled(u)))return T('当前战线需要可行动的'+({Red:'红',Blue:'蓝',Green:'绿',Black:'黑'}[c.color]||'对应')+'色英雄','This lane needs a ready '+({Red:'red',Blue:'blue',Green:'green',Black:'black'}[c.color]||'matching')+' hero');return '';}
 targets(k,p,source=null){return targetSpec(this.card(k),p,source);}
 validateTargets(c,p,t,source=null){const spec=this.targets(c.key,p,source);for(let n=0;n<spec.length;n++){const s=spec[n],v=t[n];if(s.kind==='lane'){if(!Number.isInteger(v)||v<0||v>2)throw Error(T('请选择战线','Choose a lane'));continue;}if(s.kind==='position'){if(!v||!Number.isInteger(v.lane)||!Number.isInteger(v.pos)||v.pos< -1||v.lane<0||v.lane>2)throw Error(T('请选择空位','Choose an empty slot'));if(!s.occupied&&this.at(p,v.lane,v.pos))throw Error(T('位置已被占用','Position is occupied'));if(!this.positionTargets(p,v.lane,s,source).includes(v.pos))throw Error(T('必须优先补对位空缺；没有空缺时只能从两端扩展','Must fill the opposite slot first; with no gaps you may only extend from the edges'));if(!s.cross&&v.lane!==this.s.lane)throw Error(T('只能在当前战线召唤','Can only summon in the current lane'));continue;}if(s.kind==='improvement'){let found=false;for(let l=0;l<3;l++)for(let q=0;q<2;q++)if((s.cross||l===this.s.lane)&&(!s.enemy||q!==p)&&this.imps(q,l).some(i=>i.uid===v))found=true;if(!found)throw Error(T('请选择有效强化','Choose a valid improvement'));continue;}const u=this.get(v);if(!this.selectable(u))throw Error(T('请选择可选中的场上单位（翻牌单位无法选中）','Choose a selectable unit on the board (flipped units cannot be targeted)'));if(!s.cross&&u.lane!==this.s.lane)throw Error(T('目标必须在当前战线','Target must be in the current lane'));if(s.side==='ally'&&u.owner!==p||s.side==='enemy'&&u.owner===p)throw Error(T('目标阵营不符','Wrong allegiance'));if(s.hero&&!u.hero||s.creep&&u.hero)throw Error(s.hero?T('请选择英雄','Choose a hero'):T('请选择小兵','Choose a creep'));if(s.color&&this.card(u.k).color!==s.color)throw Error(T('目标英雄颜色不符','Wrong hero color'));if(s.other&&source&&u.uid===source.uid)throw Error(T('请选择另一个单位','Choose a different unit'));if(n>0&&t.slice(0,n).includes(v))throw Error(T('请选择不同单位','Choose a different unit'));}}
 play(p,handId,t=[]){const backup=clone(this.s);try{this.s.events=[];const pl=this.s.players[p],idx=pl.hand.findIndex(h=>h.uid===Number(handId)),h=pl.hand[idx];const why=this.canPlay(p,h);if(why)throw Error(why);const c=this.card(h.k);this.validateTargets(c,p,t);pl.hand.splice(idx,1);if(c.type!=='Item')this.s.lanes[this.s.lane].mana[p]-=c.mana;
   ...
   this.emit('card',T(`${ZSIDE(p)}打出 ${c.name}`,`${ESIDE(p)} plays ${EN(c)}`),{card:c.key,owner:p});
   ...
   else{this.resolve(c.key,p,t,h);const spec=this.targets(c.key,p);t.forEach((v,i)=>{if(spec[i]?.kind==='unit')this.mineTrigger(this.get(v),'spell',p);});pl.discard.push(c.key);}
   if(h.sabotage)this.towerDamage(p,Math.floor(this.random()*3),6);this.afterCard(p,c,h.uid,t);this.sweep();if(this.s.pendingCombat){delete this.s.pendingCombat;if(this.s.winner===null)this.combat();return this.s;}if(this.s.winner===null&&this.s.phase==='action'){this.s.passes=0;this.s.turn=c.initiative?p:1-p;if(c.initiative)this.s.initiative=p;}return this.s;
