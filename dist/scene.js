...
window.ArtifactExpansionFX=function(events,layer,game){
 ...
 const projectile=(from,to,kind,delay=0,image='')=>{if(reduced||!from||!to||from.closest('[inert]')||to.closest('[inert]'))return;const a=center(from),b=center(to),orb=document.createElement('span');orb.className='snapfire-projectile '+kind;if(image)orb.style.backgroundImage=`url(${image})`;Object.assign(orb.style,{left:a.x+'px',top:a.y+'px'});layer.appendChild(orb);const dx=b.x-a.x,dy=b.y-a.y,arc=Math.min(105,45+Math.abs(dx)*.08);orb.animate([{transform:'translate(-50%,-50%) scale(.65)',opacity:0},{transform:'translate(-50%,-50%) scale(1)',opacity:1,offset:.12},{transform:`translate(calc(-50% + ${dx*.52}px),calc(-50% + ${dy*.52-arc}px)) scale(1.18)`,offset:.55},{transform:`translate(calc(-50% + ${dx}px),calc(-50% + ${dy}px)) scale(.75)`,opacity:1}],{duration:620,delay,easing:'cubic-bezier(.2,.65,.25,1)',fill:'forwards'}).onfinish=()=>orb.remove();};
 const tracer=(from,to,delay=0)=>{if(reduced||!from||!to||from.closest('[inert]')||to.closest('[inert]'))return;const a=center(from),b=center(to),dx=b.x-a.x,dy=b.y-a.y,line=document.createElement('span');line.className='shredder-tracer';Object.assign(line.style,{left:a.x+'px',top:a.y+'px',width:Math.hypot(dx,dy)+'px',rotate:Math.atan2(dy,dx)+'rad'});layer.appendChild(line);line.animate([{scale:'0 1',opacity:0},{scale:'1 1',opacity:1,offset:.25},{scale:'1 1',opacity:.85,offset:.7},{opacity:0}],{duration:210,delay,easing:'ease-out',fill:'forwards'}).onfinish=()=>line.remove();};
 for(const e of events){
  ...
  if(e.type==='cookie'&&unit){const origin=document.querySelector(`.lane[data-lane="${e.lane}"]`)||layer.querySelector('.skill-reveal>img');projectile(origin,unit,'cookie-projectile',0,'assets/dota2/snapfire_firesnap_cookie.png');setTimeout(()=>pulse(document.querySelector(`[data-unit="${e.unit}"]`),'cookie-ring'),420);}
  if(e.type==='shredder'&&unit){for(const [n,id] of (e.targets||[]).entries()){const target=document.querySelector(`[data-unit="${id}"]`);for(let hit=0;hit<(e.hits||3);hit++)tracer(unit,target,n*75+hit*95);setTimeout(()=>pulse(document.querySelector(`[data-unit="${id}"]`),'shredder-ring'),n*75+240);}}
  if(e.type==='mortimer-spit'&&unit){const origin=document.querySelector(`.lane[data-lane="${e.fromLane}"]`);projectile(origin,unit,'lava-projectile',(e.shot||0)*190,'assets/dota2/snapfire_mortimer_kisses.png');for(const id of e.affected||[])setTimeout(()=>pulse(document.querySelector(`[data-unit="${id}"]`),'lava-ring'),(e.shot||0)*190+500);}
  if(e.type==='cookie-toss'&&unit){const slot=document.querySelector(`.lane[data-lane="${e.toLane}"] .slot[data-owner="${e.owner}"][data-slot="${e.pos}"]`);projectile(unit,slot,'cookie-toss-projectile',0,'assets/dota2/snapfire_firesnap_cookie.png');setTimeout(()=>pulse(document.querySelector(`.lane[data-lane="${e.toLane}"] .slot[data-owner="${e.owner}"][data-slot="${e.pos}"]`),'cookie-ring'),500);}
  if(e.type==='lick'&&unit){pulse(unit,'lick-ring');for(const [n,id] of (e.cleansed||[]).entries()){const t=document.querySelector(`[data-unit="${id}"]`);if(t&&t!==unit){tracer(unit,t,n*60);setTimeout(()=>pulse(t,'lick-ring'),n*60+330);}}for(const [n,h] of (e.pulls||[]).entries()){const t=document.querySelector(`[data-unit="${h.unit}"]`);if(t)setTimeout(()=>pulse(t,'lick-ring'),n*60+330);}}
  if(e.type==='frog-bounce'){let prev=null;for(const [n,id] of (e.hits||[]).entries()){const t=document.querySelector(`[data-unit="${id}"]`);if(!t)continue;if(prev)tracer(prev,t,n*110);else if(e.lane!==undefined){const laneEl=document.querySelector(`.lane[data-lane="${e.lane}"]`);if(laneEl)projectile(laneEl,t,'frog-projectile',0,'assets/dota2/largo_frogstomp.png');}setTimeout(()=>pulse(t,'frog-ring'),n*110+140);prev=t;}}
  if(e.type==='harmony'&&unit)pulse(unit,'harmony-ring');
  if(e.type==='harmony-echo'&&e.lane!==undefined)for(const u of game.all(null,e.lane)||[])pulse(document.querySelector(`[data-unit="${u.uid}"]`),'harmony-ring');
  if(e.type==='frog-energy'&&e.owner!==undefined){const frog=(game.s.units||[]).find(v=>v.k==='largo'&&v.owner===e.owner);if(frog)pulse(document.querySelector(`[data-unit="${frog.uid}"]`),'frog-ring');}
