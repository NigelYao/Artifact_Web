...
G.canActivate=function(p,uid,k){
 ...
 if(['blink_dagger','winter_wyvern','meepo','phase_boots','monkey_king_spring'].includes(k)&&!this.canMove(u))return '缠绕期间无法移动';
 if(k==='dark_willow'&&this.flag(u,'shadowRealm'))return '已经处于暗影之境';
 if(k==='monkey_king'&&(u.charges||0)<3)return '棒击蓄势需要 3 点能量（当前 '+(u.charges||0)+' 点）';
 ...
 if(!this.candidateTargets(k,p,u).some(t=>{try{this.validateTargets({key:k},p,t,u);return !(k==='dark_seer'&&t[1]===l)&&!(k==='blink_dagger'&&t[0]===l);}catch{return false;}}))return '没有可选择的有效目标';
 return '';
};
G.activate=function(p,uid,k,t=[]){const backup=clone(this.s);try{
 ...
 if(k==='monkey_king'&&(u.charges||0)<3)throw Error('棒击蓄势需要 3 点能量');
 if(k==='monkey_king_spring'&&!this.canMove(u))throw Error('缠绕或翻牌期间无法移动');
 this.validateTargets({key:k},p,t,u);const a=this.get(t[0]),b=this.get(t[1]),it=Object.values(u.items||{}).find(i=>i.k===k);
 this.emit('ability',this.card(u.k).name+' · '+ability.name,{unit:uid,card:this.card(k)?k:u.k,skill:k});
 switch(k){
 ...
 case 'monkey_king':{u.charges-=3;this.damage(a,4,false,u.uid);this.s.players[p].monkeyBonus=(this.s.players[p].monkeyBonus||0)+1;this.emit('buff','此后召唤的猴子猴孙攻击永久 +1（当前 +'+this.s.players[p].monkeyBonus+'）',{unit:u.uid});break;}
